...
| `-lm, --load-mode MODE` | model loading mode (default: auto)<br/>- auto: mmap, unless a device does not support it<br/>- none: no special loading mode<br/>- mmap: memory-map model (if mmap disabled, slower load but may reduce pageouts if not using mlock)<br/>- mlock: force system to keep model in RAM rather than swapping or compressing<br/>- mmap+mlock: mmap + force system to keep model in RAM rather than swapping or compressing<br/>- dio: use DirectIO if available<br/>mlock and mmap+mlock do not pin the tensors that --lazy-mode reads on demand<br/><br/>(env: LLAMA_ARG_LOAD_MODE) |
...
-   `-lm MODE, --load-mode MODE`: Specify the model loading mode (default: `auto`).
    ...
    -   `mmap+mlock`: Memory-map the model and lock it in memory. The tensors that `--lazy-mode` reads on demand are left out of the locked ranges, so a big table (per-layer n-gram embeddings) can keep streaming from disk while the rest of the model stays in RAM.
